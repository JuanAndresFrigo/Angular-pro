...
describe('AppComponent', () => {
  ...
  it("should contain the 'buy me a beer' link ", () => {
    ...
    const expectedTitle: string = 'Buy me a beer';
    const expectedHref: string = 'https://www.buymeacoffee.com/scottwindon';

    expect(aElement?.title).toBe(expectedTitle);
    expect(aElement?.href).toBe(expectedHref);
    expect(aElement?.getAttribute('href')).toBe(expectedHref);
  });
});
